...
namespace Bau.Controls.Files.List
{ /// <summary>
	...
	{ // Enumerados públicos
		...
		// Delegados
			public delegate void ItemDoubleClickHandler(object objSender, ListViewItem lsiItem);
		// Eventos
			public event ItemDoubleClickHandler ItemDoubleClick;
		// Variables privadas
			private int intLastSortColumn = -1;
			private bool blnSortable = true;
			private bool blnGroupByDoubleClick = true;
			...
			Dictionary<String, List<ListViewItem>> dctMap = new Dictionary<String, List<ListViewItem>>();
				...
				foreach (ListViewItem lsiItem in Items) 
					{	string strKey = lsiItem.SubItems[intGroupColumn].Text;
							...
							if (intGroupColumn == 0 && strKey.Length > 0)
		...
		{ ListViewItem lsiItem = FocusedItem;
				...
				// Si se ha encontrado algún elemento, lanza el evento
					if (lsiItem != null && ItemDoubleClick != null)
						ItemDoubleClick(this, lsiItem);
				// Lanza el evento base
		...
		{ get { return blnSortable; }
			set { blnSortable = value; }
		}
		...
		{ get { return blnGroupByDoubleClick; }
			set { blnGroupByDoubleClick = value; }
		}		
	}
}